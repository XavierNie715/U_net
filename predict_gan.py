...
if __name__ == '__main__':
    ...
    model = Pix2PixModel(generator_net=Generator, discriminator_net=Discriminator, device=device, is_train=False)
    model.eval()

    MSE_criterion = nn.MSELoss()
    l1_criterion = nn.L1Loss(reduction='none')

    logging.info(f'Loading model {args.model} from epoch {epoch}')
    model.load_networks(epoch, sv_dir)
    ...
    for filename in os.listdir(in_files):
        ...
        with torch.no_grad():
            model.forward()
            T_pred = temp_recover(T_true_gs, model.fake_B.cpu().numpy())
        ...
        plot_and_save(filename, OH, SVF, T_pred.reshape(789, 113),
                      T_true_gs.reshape(789, 113), sv_name)
