...
from utils.utils import threshold_mask, RelativeL2Error, temp_recover
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(in_files):
        ...
        L2_error_plot = L2_criterion(torch.from_numpy(T_pred).to(device),
                                     torch.from_numpy(T_true_gs).to(device).reshape(1, -1,
                                                                                    T_pred.shape[2],
                                                                                    T_pred.shape[3]),
                                     reduct='none')
        ...
        MSE_mask_error = MSE_criterion(torch.from_numpy(T_pred).to(device) * mask,
                                       torch.from_numpy(T_true_gs).to(device).reshape(1, -1, T_pred.shape[2],
                                                                                      T_pred.shape[3]) * mask)
